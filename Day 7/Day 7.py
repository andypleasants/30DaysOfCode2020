...
#I'm going to treat this as a graph traversal problem using an assumption
#that there are no cycles

#I first need to build an adjacency matrix to show which bags
#can be DIRECTLY inside which other bags.  
#The first part of each line shows us every node in the graph

split_input = input.splitlines()

#Sort of an DFT implementation
def determine_possible_containers(possible_containers, current_container):
    #First of all lets add all the direct containers to the list
    possible_containers = possible_containers.union(adjacency_matrix[current_container])
    #Now loop over them for traversal...
    for next_bag in adjacency_matrix[current_container]:
        #If it's None - it is a "top level" bag
        if adjacency_matrix.get(next_bag) != None:
            #Now traverse the next "node"
            possible_containers = possible_containers.union(
                determine_possible_containers(possible_containers, next_bag))
    return possible_containers

adjacency_matrix = dict()

outer_bag_type_regex = re.compile(r'^\w+\s\w+')
inner_bag_types_regex = re.compile(r'(?:\d )(?P<bag>\w+\s\w+)')

for line in split_input:
    outer_bag_type = outer_bag_type_regex.findall(line)
    inner_bag_types = inner_bag_types_regex.findall(line)
    
    for inner_bag_type in inner_bag_types:
        if adjacency_matrix.get(inner_bag_type) == None:
            adjacency_matrix[inner_bag_type] = []
        adjacency_matrix[inner_bag_type].append(outer_bag_type[0])

starting_bag = 'shiny gold'
#Using a set ensures uniqueness
x = determine_possible_containers(set(), starting_bag)
print(len(x))