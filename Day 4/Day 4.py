...
#Now we need to split at double new lines to accomodate the format
split_input = input.split('\n\n')

count = 0
for passport in split_input:
    split_passport = re.split(r'[\n\s]', passport)
    if len(split_passport) == 8:
        #If there's 8 fields all fields are present and is valid
        count += 1
    elif len(split_passport) == 7:
        #If there's only 7 we need to make sure the only missing one is cid
        #Filter the fields down to just cid, if it exists, it's invalid
        cid = list(filter(lambda field: field.startswith('cid'), split_passport))
        if len(cid) == 0:
            count += 1
    else:
        pass
