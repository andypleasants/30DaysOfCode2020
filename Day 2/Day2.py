...
#parse the input with a regex - used named groups to get each section
regex = re.compile(r'^(?P<min_recurrences>\d+)(?:-)(?P<max_recurrences>\d+)(?: )(?P<char>\w)(?:\: )(?P<password>\w+)')

count = 0
for password in split_input:
    parsed = regex.match(password)

    min = int(parsed.group('min_recurrences'))
    max = int(parsed.group('max_recurrences'))
    char = parsed.group('char')
    password = parsed.group('password')

    #Count the number of char's in password
    occurrences = password.count(char)
    if min <= occurrences <= max:
        #If it is between the min and max allowed values; count it!
        count += 1
print(count)